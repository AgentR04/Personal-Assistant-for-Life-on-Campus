...
type StudentProfile = {
  name: string;
  collegeName: string;
  branch?: string; // Optional for admin/mentor
  department?: string; // For admin/mentor
  year?: string; // Student only
  hostelResident?: boolean; // Student only
  interests: string[];
  role?: "student" | "admin" | "mentor";
};

const STUDENT_ONBOARDING_QUESTIONS = [
  {
    key: "name",
    question:
...
const ADMIN_MENTOR_ONBOARDING_QUESTIONS = [
  {
    key: "name",
    question:
      "👋 Welcome to **P.A.L.** — your Personal Assistant for Life on Campus!\n\nI see you're joining as a staff member. To help me assist you better, **what's your name?**",
    placeholder: "Enter your name...",
    type: "text" as const,
  },
  {
    key: "department",
    question:
      "Nice to meet you, {name}! 🤝\n\n**Which department or office do you belong to?**",
    placeholder: "e.g. Administration, Computer Science, Student Welfare...",
    type: "text" as const,
  },
  {
    key: "interests",
    question:
      "One last thing! 🎯\n\n**What are your areas of interest or expertise?** *(Select all that apply)*",
    placeholder: "",
    type: "multiselect" as const,
    options: [
      "📚 Academics",
...
export function ChatInterface({
  ...
  const [userRole, setUserRole] = useState<"student" | "admin" | "mentor">(
    "student",
  );
  ...
  const questions =
    userRole === "student"
      ? STUDENT_ONBOARDING_QUESTIONS
      : ADMIN_MENTOR_ONBOARDING_QUESTIONS;
  ...
  useEffect(() => {
    if (!loading && initialMessage && !initialized.current) {
  ...
  const getTestModeResponse = (query: string): string => {
    ...
    const responses: Array<{ keywords: string[]; answer: string }> = [
      ...
      {
        keywords: ["abc", "parakh", "nptel", "digilocker"],
        answer:
          "**Government Portals:**\n\nI can guide you through the registration process for ABC, NPTEL, or PARAKH. \n\nCheck the **Gov Portals** page from the sidebar for a step-by-step checklist!",
      },
      {
  ...
  const checkProfileAndInit = async () => {
    ...
    if (isTestMode()) {
      ...
      if (savedProfile) {
        ...
        setMessages([
          {
            id: "onboard-0",
            role: "assistant", // Default to student flow in test mode
            content: STUDENT_ONBOARDING_QUESTIONS[0].question,
            timestamp: new Date(),
          },
        ]);
        setLoading(false);
    ...
    try {
      ...
      if (data?.profileComplete && data?.profile) {
        setStudentProfile({ ...data.profile, role });
        setProfileComplete(true);
        setOnboardingStep(-1);
        initializeChat(true);
        return;
      ...
      // Profile not complete — start onboarding with correct role
      setOnboardingStep(0);
      const qSet =
        role === "student"
          ? STUDENT_ONBOARDING_QUESTIONS
      ...
      // Fallback to student flow if error
      setOnboardingStep(0);
      setMessages([
        {
          id: "onboard-0",
  ...
  const handleOnboardingAnswer = async (answer: string) => {
    ...
    if (currentQ.key === "name") newProfile.name = answer;
    if (currentQ.key === "collegeName") newProfile.collegeName = answer;
    if (currentQ.key === "branch") newProfile.branch = answer;
    if (currentQ.key === "department") newProfile.department = answer; // Admin
    if (currentQ.key === "year") newProfile.year = answer;
    if (currentQ.key === "hostelResident")
      newProfile.hostelResident = answer.toLowerCase() === "yes";
    if (currentQ.key === "interests") newProfile.interests = answer.split(", ");
    ...
    if (nextStep < questions.length) {
      ...
      if (isTestMode()) {
        ...
        try {
          // If admin, map department to branch for backward compatibility?
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: "onboard-complete",
          role: "assistant",
          content: `✅ **Profile saved!** Here's what I know about you:\n\n${summary}\n\nI'll personalize all my responses based on your profile. **Ask me anything!**`,
          timestamp: new Date(),
        },
      ]);
    }
  ...
  const sendMessage = async (text: string) => {
    ...
    if (onboardingStep >= 0 && onboardingStep < questions.length) {
  ...
  const renderOnboardingInput = () => {
    if (onboardingStep < 0 || onboardingStep >= questions.length) return null;
  ...
  return (
    <div
      className={`flex h-[calc(100vh-4rem)] flex-col overflow-hidden ${className}`}
    >
      {/* Chat header */}
      ...
      <div className="shrink-0 border-t border-border/50 bg-background/80 px-6 py-4 backdrop-blur-sm">
        <div className="mx-auto max-w-3xl">
          ...
          {(onboardingStep < 0 ||
            questions[onboardingStep]?.type === "text") && (
            <form
              onSubmit={(e) => {
                e.preventDefault();
                sendMessage(input);
            ...
            >
              <input
                ...
                placeholder={
                  onboardingStep >= 0
                    ? questions[onboardingStep]?.placeholder
