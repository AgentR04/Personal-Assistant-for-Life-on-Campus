...
export const redisClient = createClient({
  ...
  socket: {
    ...
    host: "redis-12493.c252.ap-southeast-1-1.ec2.cloud.redislabs.com",
    port: 12493,
  },
});
...
export const connectRedis = async (): Promise<boolean> => {
  try {
    ...
    logger.warn(
      "⚠️  Redis connection failed - running without cache (development mode)",
    );
    logger.warn(
      "   To enable Redis: docker-compose up redis OR install Redis locally",
    );
    return false;
  }
};
