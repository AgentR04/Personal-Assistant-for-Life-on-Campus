...
class DocumentService {
  ...
  /**
   * Register a pre-uploaded document (e.g. via UploadThing)
   */
  async registerDocument(input: RegisterDocumentInput): Promise<Document> {
    const { userId, documentType, fileUrl, fileName, fileSize } = input;
    ...
    try {
      ...
      logger.info(`Document registered: ${document.id} via ${fileUrl}`);
  ...
  private async processDocument(
    ...
    try {
      ...
      const { data: extractedData, confidence } =
        await VisionService.extractFields(imageBuffer, mimeType, documentType);

      logger.info(
        `Extracted data from ${documentId} with confidence ${confidence}`,
      );

      // Step 3: Validate extracted data
      const validation = VisionService.validateExtractedData(
        extractedData,
        documentType,
      );
      ...
      logger.info(
        `Document ${documentId} processed successfully with status: ${status}`,
      );
      ...
      if (document) {
        ...
        if (status === "green") {
          await UserService.updateTaskProgress(
            document.user_id,
            documentId,
            "completed",
          );
  ...
  private async sendDocumentNotification(
    ...
    try {
      ...
      logger.info(
        `Notification queued for document ${document.id} with status ${status}`,
      );
    } catch (error) {
