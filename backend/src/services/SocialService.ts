...
class SocialService {
  ...
  async submitInterests(
    ...
    try {
      ...
      await supabaseAdmin.from("interests").delete().eq("user_id", userId);
      ...
      await supabaseAdmin.from("interests").insert(interestRecords);
    ...
    try {
      ...
      for (const candidate of potentialMatches) {
        ...
        const candidateTags = new Set(
          candidate.interests.flatMap((i: any) => i.tags),
        );

        const intersection = new Set(
          [...userTags].filter((x) => candidateTags.has(x)),
        );
        ...
        if (user.hostel && candidate.hostel === user.hostel)
          proximityScore += 0.4;
        ...
        const matchScore =
          interestScore * 0.4 + proximityScore * 0.3 + engagementScore * 0.3;
  ...
  private async storeMatches(
    ...
    try {
      ...
      const existingIds = new Set(
        existing?.map((m) => m.matched_user_id) || [],
      );
      ...
      if (newMatches.length > 0) {
        await supabaseAdmin.from("matches").insert(newMatches);
      }
    } catch (error) {
